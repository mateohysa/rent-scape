...
const FiltersFull = () => {
  ...
  // Convert square feet values to square meters for display
  const squareFeetToMeters = (sqFeet: number | null) => {
    if (sqFeet === null) return null;
    return Math.round(sqFeet * 0.0929);
  };

  // Convert square meters back to square feet for filtering
  const squareMetersToFeet = (sqMeters: number | null) => {
    if (sqMeters === null) return null;
    return Math.round(sqMeters / 0.0929);
  };
  ...
  // Calculate square meter values for display
  const sqMetersMin = squareFeetToMeters(localFilters.squareFeet[0]);
  const sqMetersMax = squareFeetToMeters(localFilters.squareFeet[1]);
      ...
      <div className="flex flex-col space-y-6">
        ...
        <div>
          ...
          <div className="flex justify-between mt-2">
            <span>€{Math.round((localFilters.priceRange[0] ?? 0) * 0.92)}</span>
            <span>€{Math.round((localFilters.priceRange[1] ?? 10000) * 0.92)}</span>
          </div>
        </div>
        ...
        {/* Square Meters */}
        <div>
          <h4 className="font-bold mb-2">Square Meters</h4>
          <Slider
            min={0}
            max={465} // ~5000 sq ft in square meters
            step={10}
            value={[
              sqMetersMin ?? 0,
              sqMetersMax ?? 465,
            ...
            onValueChange={(value) => 
              setLocalFilters((prev) => ({
                ...prev,
                squareFeet: [
                  squareMetersToFeet(value[0]),
                  squareMetersToFeet(value[1])
                ] as [number, number],
              }))
            }
          ...
          />
          <div className="flex justify-between mt-2">
            <span>{sqMetersMin ?? 0} m²</span>
            <span>{sqMetersMax ?? 465} m²</span>
          </div>
        </div>
